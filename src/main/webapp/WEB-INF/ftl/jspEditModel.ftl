							...
							<div class="tab-content">
								...
								<div class="table-responsive">
									<form id="form1">
											...
											<!-- ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ 固定模板 ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ -->
												<tr>
													...
													<td><select id="status" name="status"
														style="width: 200px;" tabindex="2">
															<option value="1" <c:if test="${r'${modelMap.status'} == 1 }">selected="selected"</c:if> >正常</option>
															<option value="0" <c:if test="${r'${modelMap.status'} == 0 }">selected="selected"</c:if> >删除</option>
													</select></td>
												</tr>
												<tr>
													<th>图片</th>
													<td>
														<div>
															<input type="file" name="files" class="imgUpload" /> <img
																	...
																	src="${r'${ctxImg}'}${r'${modelMap.img}'}" onerror="imgError(this);"
																	style="max-width: 600px; max-height: 200px;" /> <input
																	type="hidden" name="baseImg" class="uploadFile" /> <input
																	type="hidden" name="img" id="img"
																	value="${r'${modelMap.img}'}" />
														</div>
													</td>
												</tr>
												<tr>
													...
													<td><input type="text" id="name" name="name"
														class="form-control-row-350" value="${r'${modelMap.name}'}"/></td>
												</tr>
												<tr>
													<th>描述</th>
													<td><textarea id="desc" name="desc"
															class="form-control-row">${r'${modelMap.desc}'}</textarea></td>
												</tr>
												<tr>
													<th>状态</th>
													<td>
														<div class="radio i-checks">
															<label> <input type="radio" value="1"
																name="status" <c:if test="${r'${modelMap.status'} == 1 }">checked="checked"</c:if> > <i></i> 正常
															</label><label> <input type="radio" value="0"
																name="status"<c:if test="${r'${modelMap.status'} == 0 }">checked="checked"</c:if> > <i></i> 删除
															</label>
														</div>
													</td>
												</tr>
    											...
    											<tr>
													...
													<td><input type="text" id="${pro.proName}" name="${pro.proName}"
														class="form-control-row-350" value="${r'${modelMap.'}${pro.proName}${r'}'}"/></td>
												</tr>
												</#if>
												</#list>
												<!--	↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑ 数据库各字段信息 ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑ -->
										...
										<input type="hidden" id="id" name="id" value="${r'${modelMap.id}'}" />
									</form>
								</div>
							</div>
						</div>
